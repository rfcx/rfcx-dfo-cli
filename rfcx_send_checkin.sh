...
AUDIO_ORIG_FILEPATH=$1;
GUARDIAN_GUID="c9d541166e12"

AUDIO_ORIG_FILENAME=$(basename -- "$AUDIO_ORIG_FILEPATH")
read -r DATETIME_ISO DATETIME_EPOCH_ <<< "$(date -jf "$DATETIME_FILENAME_FORMAT" '+%Y-%m-%dT%H:%M:%S.000%z %s' "${AUDIO_ORIG_FILENAME:8:19}")"
